<form action="<?php echo base_url('mesin/update/' . $detail['id']); ?>" method="post" enctype="multipart/form-data">
    <!-- <div class="col-12">
    ...
    <div class="col-12">
        <label for="subjek" class="form-label font-kecil">Kerusakan</label>
        <input type="text" name="kerusakan" id="kerusakan" class="form-control font-kecil" placeholder="Ketik Kerusakan" required>
        <input type="hidden" name="rusak_id" id="rusak_id">
        <input type="hidden" name="id" id="id" value="<?= $detail['id']; ?>">
        <input type="hidden" name="dept_id" id="dept_id" value="<?= $detail['dept_id']; ?>">
    </div>
    <div class="col-12">
        <label for="keterangan" class="form-label font-kecil">Keterangan Detail </label>
        <textarea class="form-control font-kecil" name="keterangan" id="keterangan" style="height: 100px"></textarea>
    </div>
    <div class="col-12">
        <label for="file_upload" class="form-label font-kecil">Upload( Foto, Video)</label>
    ...
    <br>
    <div class="col-3">
        <button class="btn btn-primary w-100" type="submit">Perbaiki</button>
    </div>
</form>
...
<script>
    $("form").on("submit", function(e) {
        ...
        if (!kerusakan || !rusak_id) {
            e.preventDefault();
            showError("Silakan Pilih Jenis Kerusakan Dari Data Yang Telah Di Sediakan.");
            return false;
        }
    });
</script>