...
<div class="modal fade" id="basicModal-edit" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Deskripsi Kerusakan</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
...
<div class="modal fade" id="basicModal-kesimpulan" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Kesimpulan Perbaikan</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
...
<script>
  $(document).ready(function() {
    $("#tambahdata").click(function(e) {
      ...
      const url = "<?= base_url('mesin/tambahdata/'); ?>" + filter_dept;
      $("#basicModal").modal("show");
      $("#loadforminput").load(url, function() {
        $("#nomesin").autocomplete({
          source: function(request, response) {
            $.ajax({
              url: "<?= base_url('mesin/nomor_mesin'); ?>",
              dataType: "json",
              data: {
                term: request.term,
                filter: $('#filter').val()
        ...
        $("#kerusakan").autocomplete({
          source: function(request, response) {
            $.ajax({
              url: "<?= base_url('mesin/kerusakan_mesin'); ?>",
              dataType: "json",
              data: {
    ...
    $(document).on("click", ".view-file", function(e) {
      ...
      if (['jpg', 'jpeg', 'png', 'gif'].includes(ext)) {
        content = '<img src="' + fileUrl + '" style="width:100%; height:600px; " class="img-fluid" />';
      } else if (['pdf', 'mp4'].includes(ext)) {
    ...
    $(document).on('click', '.edit', function() {
      ...
      if (data) {
        ...
        $("#loadforminput-edit").load("<?= base_url(); ?>mesin/edit/" + encodeURIComponent(data), function() {
          $("#kerusakan").autocomplete({
            source: function(request, response) {
              $.ajax({
                url: "<?= base_url('mesin/kerusakan_mesin'); ?>",
    ...
    $(document).on('click', '.kesimpulan', function() {
      ...
      if (data) {
        ...
        $("#load-kesimpulan").load("<?= base_url(); ?>mesin/kesimpulan/" + encodeURIComponent(data));
      } else {
        alert("Data tidak valid!");
      }
    });
...
<!-- <script>
  $(document).ready(function() {
    var table = $('#downtimeTable').DataTable({
      ...
      stateSave: true,
      ajax: {
        url: "<?= base_url('mesin/filter_downtime') ?>",
        type: "POST",
        data: function(d) {
  ...
  $(document).ready(function() {
    ...
    function loadData() {
      $.ajax({
        ...
        success: function(res) {
          ...
          let startData = (currentPage - 1) * limit + 1;
          let endData = startData + res.data.length - 1;

          $('#infoData').text(`Showing ${startData} to ${endData} of ${res.recordsFiltered}`);

          $('#prevPage').prop('disabled', currentPage === 1);
          $('#nextPage').prop('disabled', endData >= res.recordsFiltered);
        }
      });
    }
    ...
    function renderCard(data) {
      ...
      data.forEach(function(row) {
        ...
        let statusClass = row.status.toLowerCase().includes('selesai') ? 'bg-success' : 'bg-warning text-dark';
