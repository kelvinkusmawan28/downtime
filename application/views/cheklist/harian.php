        ...
        <div class="col-lg-12 ">
            ...
            <div class="card h-100">
                ...
                <div class="card-body font-kecil ">
                    ...
                    <form action="<?= base_url('cheklist/simpan_harian') ?>" method="post">
                        <div class="row">
                            <div class="col-lg-9 mt-3">
                                ...
                                <?php endif; ?>
                                <?php if (!$sudah_cek) : ?>
                                    <button type="button" id="btnLibur" class="btn btn-sm btn-danger text-dark mb-2">
                                        ✔ Libur | Mesin OFF
                                    </button>
                                <?php endif; ?>
                            </div>
                            ...
                            <div class="col-lg-4">
                                <?php if ($ada_terlewat) : ?>
                                    <div class="alert alert-warning ">
                                        <span style="font-size: 22px;"> ⚠️</span> <b style="color:black;">PERINGATAN CHECKLIST HARIAN</b><br>
                                        <ul class="text-danger">
                                            <?php foreach ($tanggal_terlewat as $tgl) : ?>
                                                <li><?= $tgl ?></li>
                                            <?php endforeach; ?>
                                        </ul>
                                        <span style="font-size: 14px; color:black">Checklist Belum Terisi !</span>
                                    </div>
                                <?php endif; ?>
                            ...
                            <table class="table table-bordered">
                                <thead>
                                    <tr class="bg-info">
                                        ...
                                        <th style="color: black;">Standar</th>
                                        <th style="color: black;">OK</th>
                                        <th style="color: black;">NO</th>
                                        <th style="color: black;">LB</th>
                                ...
                                <tbody>
                                    ...
                                    foreach ($items as $row) : ?>
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <input type="radio" class="radio-lb" name="status[<?= $row->id_item ?>]" value="LB" <?= ($row->status == 'LB') ? 'checked' : '' ?> <?= ($sudah_cek) ? 'disabled' : '' ?>>
                                            </td>
                                            <td>
                        ...
                        <div class="row mt-4">
                            <?php if (!$sudah_cek) : ?>
                                ...
                                <?php else : ?>
                                    <div class=" col-lg-12 mt-5 " style="text-align: center; color:red;">
                                        ✔ <b>PREVENTIF HARIAN SUDAH DI CEK</b><br>
                                        Petugas : <b><?= $info_cek['petugas'] ?></b><br>
                                        Tanggal : <?= format_tanggal_indonesia($info_cek['tanggal']); ?>
                                    </div>
...
<script>
    $('#tanggal').change(function() {
        let tgl = $(this).val();
        let url = "<?= base_url('cheklist/harian/' . $header['mach_id'] . '/' . $kode['kode']) ?>?tanggal=" + tgl;
...
<script>
    document.getElementById('btnLibur')?.addEventListener('click', function() {
        document.querySelectorAll('.radio-lb:not(:disabled)').forEach(function(el) {
            el.checked = true;
        });
